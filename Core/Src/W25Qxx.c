...
//==============================================
uint32_t* MISO_BB = (uint32_t*) 0x42408208;
//==============================================
#define F_CLK_LO GPIOB->BSRR =  (1 << 16);
#define F_CLK_HI GPIOB->BSRR =  1;
#define F_DO_LO GPIOF->BSRR = (1 << 27);
#define F_DO_HI GPIOF->BSRR = (1 << 11);

#define F_DATA_IN MISO_BB[0];
//==============================================
...
void SPI_Write (uint8_t *data, uint32_t len)
{
	...
	for(x=0; x<len ; x++){
		dana = data[x];
		for(i=0; i < 8 ; i++) {
			if((dana & 0x80) != 0) {F_DO_HI;} 			// MSB TO MOSI
				else {F_DO_LO;}
			F_CLK_LO;
			dana =(dana << 1);
			F_CLK_HI;
